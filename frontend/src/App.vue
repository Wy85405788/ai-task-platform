...
<!--    <aside-->
<!--      class="bg-white border-r border-slate-200 flex flex-col shadow-sm transition-all duration-300 relative"-->
<!--      :class="[isCollapsed ? 'w-16' : 'w-80']"-->
<!--    >-->
<!--      <button-->
<!--        @click="isCollapsed = !isCollapsed"-->
<!--        class="absolute -right-3 top-20 bg-white border border-slate-200 rounded-full p-1 shadow-md hover:text-blue-600 z-50 transition-transform"-->
...
<!--      <div-->
<!--        class="p-6 border-b border-slate-100 flex items-center justify-between overflow-hidden whitespace-nowrap">-->
<!--        <div class="flex items-center gap-2">-->
<!--          <div class="p-1.5 bg-blue-600 rounded-lg text-white shrink-0">-->
<!--            <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24"-->
<!--                 fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round"-->
<!--                 stroke-linejoin="round">-->
...
<!--            </svg>-->
<!--          </div>-->
<!--          <h2 v-show="!isCollapsed" class="text-lg font-bold tracking-tight text-slate-800">-->
<!--            挑战历史</h2>-->
<!--        </div>-->
...
<!--          class="p-2 hover:bg-slate-100 rounded-full text-slate-500 hover:text-blue-600"-->
<!--        >-->
<!--          <svg v-if="!isStreaming" xmlns="http://www.w3.org/2000/svg" width="20" height="20"-->
...
<!--            <path d="M8 12h8"/>-->
<!--          </svg>-->
<!--          <svg v-else class="animate-spin" xmlns="http://www.w3.org/2000/svg" width="20" height="20"-->
<!--               viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"-->
<!--               stroke-linecap="round" stroke-linejoin="round">-->
...
<!--            class="w-full text-left p-4 rounded-2xl transition-all border border-transparent group relative overflow-hidden whitespace-normal"-->
<!--            :class="[task?.id === hTask.id ? 'bg-blue-50 border-blue-200 shadow-sm' : 'hover:bg-slate-50']"-->
<!--          >-->
<!--            <p class="text-sm font-bold text-slate-700 line-clamp-2 leading-snug">-->
<!--              {{ hTask.description }}</p>-->
<!--          </button>-->
<!--        </template>-->

<!--        <div v-else class="flex flex-col items-center gap-4 text-slate-300">-->
<!--          <div v-for="i in 5" :key="i" class="w-8 h-8 rounded-full bg-slate-100"></div>-->
<!--        </div>-->
<!--      </div>-->
<!--    </aside>-->

<!--    <main class="flex-1 overflow-y-auto p-8 relative bg-white">-->
<!--      <div v-if="loading && (!task || !task.description)"-->
<!--           class="h-full flex flex-col items-center justify-center">-->
<!--        <div-->
<!--          class="animate-spin rounded-full h-10 w-10 border-4 border-blue-500 border-t-transparent mb-4"></div>-->
...
<!--            <span-->
<!--              class="px-3 py-1 bg-blue-50 text-blue-700 text-[10px] font-black rounded-full uppercase tracking-widest border border-blue-100">-->
...
<!--            <div-->
<!--              class="absolute -top-3 left-6 px-3 py-1 bg-slate-900 text-white text-[10px] font-bold rounded-md z-10">-->
<!--              PYTHON CODE EDITOR-->
<!--            </div>-->
<!--            <textarea-->
<!--              v-model="userCode"-->
...
<!--          >-->
<!--            <svg v-if="isChecking" class="animate-spin" xmlns="http://www.w3.org/2000/svg"-->
...
<!--        <button @click="fetchTask"-->
<!--                class="mt-6 px-8 py-3 bg-blue-600 text-white rounded-2xl font-bold shadow-lg shadow-blue-200 hover:scale-105 transition-all">-->
<!--          生成今日新挑战-->
<!--        </button>-->
<!--      </div>-->
<!--    </main>-->
...
onMounted(() => {
  taskStore.fetchHistory() // 初始化时获取一次历史记录
})
</script>
...
<template>
  <div class="flex h-screen bg-slate-50 overflow-hidden text-slate-900">
    <aside
      :class="[isCollapsed ? 'w-16' : 'w-80']"
      class="bg-white border-r border-slate-200 flex flex-col transition-all duration-300 relative"
    >
      <div v-show="!isCollapsed" class="p-4">
         <h2 class="font-bold mb-4">挑战历史</h2>
         <div v-for="hTask in taskStore.historyTasks" :key="hTask.id"
              @click="taskStore.selectTask(hTask); $router.push('/task')"
              class="p-3 mb-2 hover:bg-slate-100 rounded-xl cursor-pointer">
           {{ hTask.id }}
         </div>
       </div>
    </aside>
